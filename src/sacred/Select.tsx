...
const Select: React.FC<SelectProps> = ({ name, options, placeholder, defaultValue = '', onChange }) => {
  ...
  const [_index, setIndex] = React.useState(-1);
  ...
  return (
    <>
      <section id={name} className={styles.select}>
        <figure
      ...
      {isOpen && (
        <ul className={styles.menu} role="listbox">
          {options.map((option, idx) => {
            return (
              <li key={idx + option} role="option" tabIndex={0} className={Utilities.classNames(styles.item)} onClick={() => handleSelect(option)}>
                {option}
              </li>
            );
          })}
