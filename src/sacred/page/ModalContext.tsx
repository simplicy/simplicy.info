...
interface ModalContextType {
  ...
  open: <P>(component: ModalComponent<P>, props: P, title: string) => string;
...
export const ModalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const open = <P,>(component: ModalComponent<P>, props: P, title: string): string => {
    let key = `modal-${title}`;
    const existingModal = modalStack.find((modal) => modal.key.includes(title));
    if (existingModal) {
      return existingModal.key;
    }
